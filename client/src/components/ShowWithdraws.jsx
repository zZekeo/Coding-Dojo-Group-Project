...
const ShowWithdraws = (props) => {
    ...
    // custom function that loops through all of the expenses, and saves ONLY the withdraw expenses
    // this way our withdraw page only displays withdraws, and not the deposits as it was displaying both before
    let arr = [];
    for (let i = 0; i < allExpenses.length; i++) {
        if (allExpenses[i].withdrawName !== undefined) {
            console.log("Expense at ", i, allExpenses[i].withdrawName);
    ...
    return (
        <div className="container">
            ...
            <h1 className='text-white'>Deductions</h1><br></br>
                ...
                <tbody>
                    {
                        arr.map((eachExpense) => (
                            <tr>
                                <td><Link className='bg-transparent text-white text-decoration-none' to={`/api/oneWithdraw/${eachExpense._id}`}>{eachExpense.withdrawName}</Link></td>
                                <td>${eachExpense.withdrawAmount}</td>
            ...
            <Link className='btn btn-primary' to={'/api/newWithdraw'}>Add Deduction</Link>
        </div>
    )
}

export default ShowWithdraws;